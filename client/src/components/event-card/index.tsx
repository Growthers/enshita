...
const EventCard : FC<EventCardProperties> = ({ eventId, title, startDate, ogp, status, hashTag, speakerQuotaTypeList}) => {
  const memo = useMemo(() => {
    let current = 0;
    let total = 0;
    ...
    <Link to={`/event/${eventId}`}>
      <div className={styles["enshita-event-card"]}>
        <div className={styles["enshita-event-card-image-wrapper"]}>
          <img src={ogp} alt={title} className={styles["enshita-event-card-image"]} />
        </div>
        <div className={styles["enshita-event-card-content-wrapper"]}>
          <div className={styles["enshita-event-card-content"]}>
            ...
            <div className={styles["enshita-event-card-sub-content"]}>
              <time className={styles["enshita-event-card-date"]}>{dayjs(startDate).format("MMM DD")}</time>
          ...
          <div className={styles["enshita-event-card-option"]}>
            <p className={styles["enshita-event-card-num-of-speaker"]}>
              <span className={styles["enshita-event-card-slash"]}>{memo.current}</span>
              {memo.total}
            </p>
            <Chip status={status} />
          </div>
